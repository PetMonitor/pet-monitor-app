...
import {
  mapReportTypeToLabelColor,
  mapPetTypeToLabel,
  mapPetSexToLabel,
  mapPetSizeToLabel,
  mapPetLifeStageToLabel,
  mapReportTypeToPetLocationTitle,
  mapReportTypeToReportLabel,
} from "../utils/mappers";
...
import { validateEmail, selectedLocation } from "../utils/commons";
...
export class ReportViewScreen extends React.Component {
    ...
    getSecureStoreValueFor("sessionToken").then((sessionToken) => {
      deleteJsonData(
        global.noticeServiceBaseUrl +
          "/users/" +
          this.props.route.params.noticeUserId +
          "/notices/" +
          this.state.noticeId,
        {},
        {
  ...
  goToEdit = () => {
    this.props.navigation.push("EditReportScreen", {
      reportState: this.state,
      onUpdate: this.onReportDataUpdated,
    });
  };
  ...
  editHomeButton = (historyData) => {
    ...
    this.setState({
      ...
      homeUntilSelectedDate: new Date(
        historyData.untilDate ? historyData.untilDate : new Date()
      ),
    });
  };
  ...
  componentDidMount() {
    ...
    getSecureStoreValueFor("userId").then((userId) =>
      this.setState({
        isMyReport: userId === this.props.route.params.noticeUserId,
      })
    );
  ...
  fetchReportInfo(sessionToken) {
    getJsonData(
      global.noticeServiceBaseUrl +
        ...
        this.props.route.params.noticeUserId +
        "/notices/" +
        this.props.route.params.noticeId,
      {
        Authorization: "Basic " + sessionToken,
      }
    )
  ...
  fetchFosteringInfo(petId, sessionToken) {
    getJsonData(
      global.noticeServiceBaseUrl + "/pets/" + petId + "/fosterHistory",
      {
        Authorization: "Basic " + sessionToken,
      }
    )
      .then((history) => {
        ...
        for (let i = 0; i < history.length; i++) {
          ...
          promises.push(
            getJsonData(
              global.noticeServiceBaseUrl + "/users/" + userId + "/contactInfo"
            )
              .then((userInfo) => {
                history[i] = {
                  ...history[i],
  ...
  render() {
    ...
    const changeContactModalVisibility = () =>
      this.setContactModalVisible(!this.state.contactInfoModalVisible);
    const changeNewHomeModalVisibility = () =>
      this.setFosteringHomeModalVisible(
        !this.state.newFosteringHomeModalVisible
      );
    const changeExistingVolunteer = (value) =>
      this.setState({ existingVolunteer: value });
    ...
    return (
      <SafeAreaView style={commonStyles.container}>
        ...
        <NewFosteringHomeModal
          ...
          onSinceDateSelect={(selectedDate) =>
            this.setState({ homeSinceSelectedDate: selectedDate })
          }
          untilDate={this.state.homeUntilSelectedDate}
          onUntilDateSelect={(selectedDate) =>
            this.setState({ homeUntilSelectedDate: selectedDate })
          }
          ...
          onEditHomePress={() =>
            this.editFosterHomeData(this.state.historyDataToEdit)
          }
        ...
        <HeaderWithBackArrow
          ...
          onBackArrowPress={this.navigateToReports}
        />
        <PetImagesHeader
          petPhotos={this.state.petPhotos}
        ...
        <View style={{ flex: 2 }}>
            ...
            <Title
              ...
              isMyReport={
                ...
                this.state.selectedIndex ==
                  segmentedTabTitles.indexOf(infoTitle)
              }
          ...
          >
            <ReportContent
              ...
              reportInfo={{
                eventInfo: {
                  locationTitle: mapReportTypeToPetLocationTitle(
  ...
  return (
    ...
    >
      ...
      <EditModePencil
        ...
        onEditModePress={onEditModePress}
      />
    </View>
  );
};

const FosteringHistoryTabSeletor = ({
  segmentedTabTitles,
  selectedIndex,
  onSelectedTabPress,
  showHistoryInfo,
      ...
      >
        ...
        >
          <ContactInfo
            ...
            onContactInfoOk={onContactInfoOk}
          />
        </View>
      </Modal>
    </View>
    ...
    <View>
      ...
      >
        ...
        >
          <FosterEntryInfo
            sinceDate={sinceDate}
            onSinceDateSelect={onSinceDateSelect}
            onAddHomePress={onAddHomePress}
            ...
            existingVolunteer={existingVolunteer}
            ...
            onEditHomePress={onEditHomePress}
  ...
  return (
    <View style={styles.modalView}>
      <Text style={[styles.modalTitle, { marginBottom: 25 }]}>
        {dataToEdit ? "Editar hogar" : "Crear nuevo hogar"}
      </Text>
      {dataToEdit ? (
        ...
        <>
          <Text
            style={[styles.modalText, { fontWeight: "bold", marginBottom: 0 }]}
          >
            Fecha de entrada
          </Text>
      ...
      {dataToEdit ? (
        dataToEdit.userId == null ? (
          <>
            ...
            <Text
              style={[styles.optionTitle, { paddingTop: 5, fontWeight: "500" }]}
            >
              Nombre
            </Text>
            <OptionTextInput onChangeText={onNameChange} value={name} />
            <Text style={[styles.optionTitle, { fontWeight: "500" }]}>
              Teléfono
            </Text>
            <OptionTextInput
      ...
      <View style={[commonStyles.alignedContent, { marginTop: 20 }]}>
        ...
        />
        <AppButton
          buttonText={dataToEdit ? "Actualizar" : "Agregar"}
          onPress={dataToEdit ? onEditHomePress : onAddHomePress}
          additionalButtonStyles={{
            alignItems: "center",
            flex: 1,
            width: "50%",
  ...
  for (let i = 0; i < historyData.length; i++) {
    row.push(
      <FosterInfoRow
        ...
        untilDate={
          historyData[i].untilDate ? (
            <DateToDisplay date={historyData[i].untilDate} />
          ) : (
            <Text
              style={[styles.textInput, { fontSize: 13, alignSelf: "center" }]}
            >
              -
            </Text>
          )
...
const DateToDisplay = ({ date }) => {
  return (
    <Text style={[styles.textInput, { fontSize: 13 }]}>
...
function getDate(date) {
  return (
    fillDateWithZero(date.getDate()) +
    "/" +
    fillDateWithZero(parseInt(date.getMonth() + 1)) +
    "/" +
    date.getFullYear()
  );
}
...
const EventInfo = ({
  ...
  return (
    <>
      <OptionTitle
        text={petLocationTitle}
        additionalStyle={{ ...styles.optionTitle, paddingTop: 0 }}
      />
      <Text style={styles.textInput}>{eventLocation}</Text>
      <Text style={[styles.textInput, { paddingTop: 5 }]}>
        {(eventCity != "" ? eventCity + ", " : "") + eventProvince}
      </Text>
      ...
      <View style={[commonStyles.alignedContent, { justifyContent: "center" }]}>
        <View style={{ flexDirection: "column", flex: 0.5 }}>
          ...
          <Text style={styles.textInput}>
            {eventDate.getDate() +
              "/" +
              fillDateWithZero(parseInt(eventDate.getMonth() + 1)) +
              "/" +
        ...
        <View style={{ flexDirection: "column", flex: 0.5 }}>
          ...
          <Text style={styles.textInput}>
            {fillDateWithZero(eventHour.getHours()) +
              ":" +
              fillDateWithZero(eventHour.getMinutes())}
          </Text>
        </View>
      </View>
    ...
    <>
      ...
      <View style={commonStyles.alignedContent}>
        ...
        <View style={{ flexDirection: "column", flex: 0.5 }}>
          ...
          <OptionTitle
            text={"Color de pelaje"}
            additionalStyle={styles.optionTitle}
          />
          <Text style={styles.textInput}>{furColor}</Text>
          <OptionTitle
            text={"Etapa de la vida"}
            additionalStyle={styles.optionTitle}
          />
          <Text style={styles.textInput}>
            {mapPetLifeStageToLabel(lifeStage)}
          </Text>
        </View>
      </View>
      <OptionTitle
...
const ReportInfo = ({ eventInfo, petInfo }) => {
  return (
    <>
      <EventInfo
        petLocationTitle={eventInfo.locationTitle}
        eventLocation={eventInfo.location}
        eventCity={eventInfo.city}
        eventProvince={eventInfo.province}
        eventDate={eventInfo.date}
        eventHour={eventInfo.hour}
...
const ReportContent = ({
  ...
  if (selectedIndex == segmentedTabTitles.indexOf(infoTitle)) {
    ...
    return (
      <ReportInfo
        eventInfo={reportInfo.eventInfo}
        petInfo={reportInfo.petInfo}
      />
    );
    ...
    const title = (
      ...
      >
        {historyData.length == 0
          ? "No hay hogares registrados para esta mascota por el momento"
          : "Hogares de tránsito de la mascota"}
      </Text>
    );
    ...
    history.push(title);
    {
      historyData.length != 0 ? (
        history.push(
          <FosteringInfo historyData={historyData} onEditPress={onEditPress} />
        )
      ) : (
        <></>
      );
...
const ReportButtons = ({
  ...
  if (!isMyReport) {
    return (
      <ContactButton showContactInfo={guestButtonHandler.showContactInfo} />
    );
  } else {
    return (
      <MyReportButtons resolveReport={myReportButtonHandler.resolveReport} />
    );
  }
};
  ...
  if (isMyReport) {
    return (
      <AppButton
        ...
        additionalButtonStyles={[
          styles.button,
          { alignSelf: "center", marginTop: 30, marginBottom: 50 },
        ]}
        additionalTextStyles={{ paddingLeft: 10 }}
        additionalElement={
          <FeatherIcon name="plus" size={20} color={colors.white} />
        }
      />
    );
  }
...
const ActionButtons = ({
  ...
  if (selectedIndex == segmentedTabTitles.indexOf(infoTitle)) {
    return (
      <ReportButtons
        isMyReport={isMyReport}
        guestButtonHandler={guestButtonHandler}
        myReportButtonHandler={myReportButtonHandler}
      />
    );
  } else if (selectedIndex == segmentedTabTitles.indexOf(historyTitle)) {
    return (
      <FosterButtons
        isMyReport={isMyReport}
        fosterInfoButtonHandler={fosterInfoButtonHandler}
      />
    );
  }
  return null;
